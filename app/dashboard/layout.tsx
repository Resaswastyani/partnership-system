    ...
    <div className="min-h-screen bg-background flex relative overflow-hidden">
        ...
        <div className="p-4 h-20 border-b border-white/5 flex items-center justify-between">
          {sidebarOpen && (
            <Link href="/dashboard" className="flex items-center gap-3">
              <Image src="/fbl-logo.png" alt="FBL" width={32} height={32} className="w-8 h-8 drop-shadow-[0_0_10px_rgba(139,92,246,0.5)]" />
              <div className="hidden lg:block">
                <p className="text-white font-bold text-sm tracking-widest">FBL</p>
                <p className="text-accent text-xs font-semibold tracking-wider">PARTNER</p>
              </div>
            </Link>
          )}
          <button
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 group ${
                  isActive
                    ? 'bg-gradient-to-r from-primary/20 to-primary/5 text-white border border-primary/30 shadow-[0_0_15px_rgba(139,92,246,0.15)] glow-effect'
        ...
        <div className="p-4 border-t border-white/5 space-y-3 pb-6">
          {sidebarOpen && user && (
            <div className="px-4 py-3 bg-white/[0.02] rounded-xl border border-white/5">
              <p className="text-gray-500 text-xs mb-1 uppercase tracking-wider">Akun Anda</p>
      ...
      <div className={`${sidebarOpen ? 'ml-64' : 'ml-20'} flex-1 transition-all duration-300 relative z-10`}>
        ...
        <div className="bg-background/80 backdrop-blur-md border-b border-white/5 px-8 h-20 flex justify-between items-center sticky top-0 z-30">
          <div>
            <h1 className="text-white font-bold text-xl tracking-wide">Dashboard Utama</h1>
            <p className="text-gray-400 text-sm mt-1">Ringkasan performa affiliate Anda</p>
          </div>
          <div className="flex items-center gap-5">
            <button className="p-2 hover:bg-white/10 rounded-xl transition-colors relative group">
              <div className="absolute top-2 right-2 w-2 h-2 bg-accent rounded-full shadow-[0_0_8px_rgba(45,212,191,1)]" />
              <svg className="w-6 h-6 text-gray-400 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
            </button>
            <div className="w-10 h-10 rounded-full bg-primary/20 border border-primary/50 flex items-center justify-center text-primary font-bold shadow-[0_0_15px_rgba(139,92,246,0.3)]">
